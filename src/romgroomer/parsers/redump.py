"""Redump filename parser for disc-based systems."""

import re
from pathlib import Path
from typing import Optional, Set, List
from romgroomer.models.rom import Rom, RomRegion, RomLanguage, RomKind
from romgroomer.parsers.base import BaseParser, register_parser


class RedumpParser(BaseParser):
    """
    Parser for Redump filename conventions.
    
    Redump format is similar to No-Intro but with some differences:
    - Disc numbers: (Disc 1) instead of (Disc 1 of 3)
    - Track numbers for multi-track games
    - More detailed version information
    
    Examples:
        "Final Fantasy VII (USA) (Disc 1).bin"
        "Metal Gear Solid (USA) (Rev 1).bin"
        "Gran Turismo (Europe) (En,Fr,De,Es,It).bin"
    """
    
    # Parser metadata
    name = "redump"
    description = "Redump naming convention parser for disc-based systems"
    supported_extensions = {
        # Disc images
        ".bin", ".cue", ".iso", ".img", ".ccd", ".sub",
        ".chd", ".pbp", ".gdi", ".cdi",
        # Compressed
        ".zip", ".7z", ".rar",
    }
    
    # Region patterns (same as No-Intro)
    REGION_PATTERN = re.compile(r'\(([^)]+)\)')
    
    # Language codes (same as No-Intro)
    LANGUAGE_CODES = {
    ...
    KNOWN_TAGS = {
        ...
        "fixed", "hack", "translation",
        "virtual console", "psn", "xbla",
        "rerelease", "remaster", "special edition",
    }
    
    def parse(self, filepath: Path) -> Rom:
        """
        Parse a Redump ROM filename.
        ...
        for group in groups:
            ...
            # Check for disc number (Redump format: just "Disc 1" usually)
            disc_match = re.match(r'disc\s+(\d+)(?:\s*-\s*(.+))?', group_lower)
            if disc_match:
                disc_number = int(disc_match.group(1))
                if disc_match.group(2):
                    disc_name = disc_match.group(2).strip()
                continue
            
            # Check for track number (multi-track games)
            track_match = re.match(r'track\s+(\d+)', group_lower)
            if track_match:
                tags.add(f"Track {track_match.group(1)}")
                continue
            
            # Check if it's a language code group
    ...
    def format_filename(self, rom: Rom, include_extension: bool = True) -> str:
        """
        Format a ROM object back into Redump filename format.
        ...
        # Add disc info (Redump format: simpler)
...
register_parser("redump", RedumpParser)
