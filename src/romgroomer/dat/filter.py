...
from romgroomer.catalog.database import DatGame
...
class OneGameOneRomFilter:
    ...
    def __init__(
        ...
        if region_priority:
            self.region_priority = {
                region: 100 - i * 5 for i, region in enumerate(region_priority)
            }
        else:
        ...
        if language_priority:
            self.language_priority = {
                lang: 100 - i * 5 for i, lang in enumerate(language_priority)
            }
        else:
    ...
    def filter_games(self, games: List[DatGame]) -> tuple[List[DatGame], FilterStats]:
        ...
        filtered = []
        for base_name, group in game_groups.items():
    ...
    def _get_language_score(self, name: str) -> int:
        ...
        languages = [l.strip() for l in lang_tag.split(',')]
