...
def display_selections(selections: List[Dict]) -> None:
    ...
    table = Table(title="Available Selection Strategies", show_header=True, header_style="bold cyan")
    table.add_column("#", style="dim", width=3)
...
def select_from_list(items: List[Dict], prompt: str, allow_none: bool = False) -> Optional[Dict]:
    ...
    choice = Prompt.ask(
        f"{prompt} (1-{len(items)}{none_option})",
        default="1"
    )
...
def quick(interactive: bool, platform: str, selection: str, config_root: str):
    ...
    Examples:
        ...
        # Interactive mode
        romgroomer quick --interactive
        
        # Direct command (skips prompts)
        romgroomer quick --platform saturn --selection usa-10
    """
    ...
    console.print(Panel.fit(
        "[bold cyan]🎮 ROM Groomer Quick Build[/bold cyan]\n"
    ...
    if builds and interactive:
        ...
        if choice == "2":
            ...
            if build:
                # Run existing build
                import subprocess
                cmd = f"python3 -m romgroomer build run {build['name']}"
                console.print()
    ...
    if platform:
        selected_platform = next((p for p in platforms if p['name'] == platform or p['file'] == platform), None)
    ...
    if selections:
        ...
        if selection:
            selected_selection = next((s for s in selections if s['name'] == selection or s['file'] == selection), None)
        else:
    ...
    if selected_selection:
        ...
        console.print(f"[green]✓ Selected:[/green] All games (no filtering)")
    console.print()
    ...
    import subprocess
    cmd = f"python3 -m romgroomer build run {build_name}"
