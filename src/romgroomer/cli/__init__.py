...
def cli(ctx: click.Context, config: str, verbose: bool, quiet: bool) -> None:
    """
    ROM Groomer - Enterprise-grade ROM collection management.
...
def init(ctx: click.Context) -> None:
    """Initialize ROM Groomer configuration."""
    logger = ctx.obj["logger"]
    console = ctx.obj["console"]
    
    logger.section("Initializing ROM Groomer")
    ...
    console.print(table)
    console.print("\n[dim]Edit ~/.config/romgroomer/config.yaml to customize profiles[/dim]")
...
def validate(ctx: click.Context, source: str, dat: str) -> None:
    ...
    Example:
        romgroomer validate ~/roms/nes --dat ~/dats/nes.dat
    """
    logger = ctx.obj["logger"]
