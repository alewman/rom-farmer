...
def validate(ctx: click.Context, source: str, dat: str, check_size: bool, check_m3u: bool) -> None:
    ...
    if check_m3u:
        ...
        if m3u_files:
            ...
            for m3u in m3u_files:
                try:
                    with open(m3u, encoding="utf-8", errors="ignore") as fh:
                        for line in fh:
                            line = line.strip()
    ...
    if dat:
        ...
        logger.info(
            f"DAT contains {dat_file.get_game_count()} games, {dat_file.get_rom_count()} ROMs"
        )
        ...
        file_stems = {
            f.stem for f in all_files if f.suffix.lower() not in {".m3u", ".xml", ".txt", ".cfg"}
        }
        ...
        if missing:
            logger.info(
                f"{len(missing)} DAT entries not found on disk (normal for filtered builds):"
            )
        ...
        logger.info(
            f"DAT match: {len(matched)}/{len(dat_stems)} ({100 * len(matched) / max(len(dat_stems), 1):.0f}%)"
        )
...
@click.option(
    "--config-dir", type=click.Path(exists=True), help="Config directory (default: ./config)"
)
...
def web(host: str, port: int, config_dir: str, live_reload: bool) -> None:
    ...
    try:
        ...
        click.echo(
            "Error: uvicorn is required. Install with: pip install 'romfarmer[web]'", err=True
        )
