...
def validate_lists(ctx: click.Context, platform: str, config_dir: str):
    ...
    if total_errors > 0 or total_warnings > 0:
        ...
        console.print("[green]✓ All list files validated successfully![/green]")
...
def _validate_delete_list(list_file: Path, console: Console) -> list:
    ...
    console.print(
        f"  [dim]{list_file.name}: {len(lines)} entries (validation skipped - processed at runtime)[/dim]"
    )
    return errors
...
def _validate_extra_list(list_file: Path, extra_dir: Path, console: Console) -> list:
    ...
    if errors:
        console.print(
            f"  [red]✗ {list_file.name}: {found}/{len(lines)} found in {extra_dir.name}/[/red]"
        )
        for filename in not_found:
            console.print(f"    [red]• {filename}[/red]")
    else:
        console.print(
            f"  [green]✓ {list_file.name}: All {len(lines)} entries found in {extra_dir.name}/[/green]"
        )
