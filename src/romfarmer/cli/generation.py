...
@click.option(
    "--output-base", type=click.Path(exists=True), default=None, help="Base output directory"
)
...
def analyze(gen_name: str, output_base: str, json_out: str):
    ...
    for _match_key, plats in duplicates.items():
        ...
        if rescue_lists:
            ...
            for _match_key, plats in duplicates.items():
                ...
                for p in plats:
                    ...
                    any_game = plats[p]
                    if any_game.original_name.lower() in [
                        g.lower() for g in rescued_games
                    ] or any_game.normalized_name.lower() in [g.lower() for g in rescued_games]:
            ...
            for p in platforms:
                ...
                if removed > 0:
                    ...
                    pct = (removed / before * 100) if before > 0 else 0
                    console.print(
                        f"  {p:12s}: {before:,} -> {after:,}  (-{removed:,}, {pct:.0f}% removed)"
                    )
                    total_removed += removed
            console.print(f"  [bold]Total removals: {total_removed:,}[/bold]")

    # Top overlapping games (appear on most platforms)
    multi_plat = sorted([(k, v) for k, v in duplicates.items()], key=lambda x: -len(x[1]))
    if multi_plat:
        table2 = Table(title="Games on Most Platforms (top 20)")
        table2.add_column("Game", style="green")
        table2.add_column("Platforms", style="cyan")
        for _key, plats in multi_plat[:20]:
...
@click.option("--model", default="claude-haiku-4.5", help="Model to use for rescue evaluation")
@click.option(
    "--batch-size", default=50, type=int, help="Games per LLM batch (higher = fewer API calls)"
)
...
def rescue(
    gen_name: str, output_base: str, model: str, batch_size: int, force: bool, dry_run: bool
):
