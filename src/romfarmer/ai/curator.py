...
class PlatformCuration:
        ...
        data = {
            ...
            "tiers": {
                tier.value: {
                    ...
                    "games": [
                        g.to_dict() for g in sorted(self.by_tier(tier), key=lambda g: -g.score)
                    ],
                }
    ...
    def from_yaml(cls, yaml_str: str) -> PlatformCuration:
        ...
        games = []
        for _tier_name, tier_data in data.get("tiers", {}).items():
...
class AICurator:
    ...
    def __init__(
        ...
        self.metadata_db = (
            metadata_db or self.workspace_root / "metadata" / "database" / "romfarmer.db"
        )
    ...
    def load_curation(self, platform: str) -> PlatformCuration | None:
