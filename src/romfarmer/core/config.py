...
class OrganizationProfile(BaseModel):
    ...
    exclude_language_regions: list[str] = Field(
        default_factory=list, description="Regions to exclude from language symlinks"
    )
    organize_kinds: bool = Field(default=True, description="Organize by kind (Applications, etc)")
    keep_in_root_regions: list[str] = Field(
        default_factory=list, description="Regions to keep in root without subfolders"
    )
    keep_in_root_kinds: list[str] = Field(
        default_factory=list, description="Kinds to keep in root without subfolders"
    )
...
class LoggingConfig(BaseModel):
    ...
    log_dir: Path | None = Field(
        default=None, description="Log directory (default: ./logs in project root)"
    )
...
class RomGroomerConfig(BaseModel):
    """
    Main configuration for ROM Farmer.

    Configuration is loaded from:
    1. ~/.config/romfarmer/config.yaml (user config)
    2. /etc/romfarmer/config.yaml (system config)
    ...
    profiles: dict[str, OrganizationProfile] = Field(
        default_factory=dict, description="Organization profiles"
    )
    ...
    case_insensitive_check: bool = Field(
        default=True, description="Check for case-insensitive filename collisions"
    )

    @classmethod
    def load(cls, config_path: Path | None = None) -> "RomGroomerConfig":
        """
        Load configuration from file.

        Args:
            config_path: Path to config file (default: ~/.config/romfarmer/config.yaml)
    ...
    def get_profile(self, name: str) -> OrganizationProfile | None:
        """Get profile by name."""
