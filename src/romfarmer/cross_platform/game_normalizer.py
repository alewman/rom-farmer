...
class GameNameNormalizer:
    ...
    REGION_PATTERN = re.compile(r'\((USA?|Europe|Japan|World|Asia|Korea|En|Fr|De|Es|It|Pt|Nl|Sv|No|Da|Fi|Zh|Ja|Ko|Ru|Pl|Tr)\)', re.IGNORECASE)
    ...
    PLATFORM_HINTS = re.compile(r'\((?:PS2|Xbox|GC|GameCube|PSX|PlayStation|Saturn)\s+(?:Exclusive|Only|Version)\)', re.IGNORECASE)
    ...
    @classmethod
    def normalize(cls, game_name: str, platform: str, file_path: Optional[str] = None) -> NormalizedGame:
    ...
    @staticmethod
    def _extract_region(name: str) -> Optional[str]:
