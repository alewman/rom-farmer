...
class CacheManager:
    ...
    def get(
        ...
        if self.config.check_tool_version and tool_name:
            ...
            if current_version and entry.tool_version != current_version:
                logger.debug(f"Cache version mismatch: {entry.tool_version} vs {current_version}")
                return CacheResult(
                    hit=False,
    ...
    def store(
        ...
        zip_content_size: int | None = None,
    ) -> CacheResult:
    ...
    def get_tool_version(self, tool_name: str) -> str | None:
        ...
        try:
            if tool_name == "chdman":
                result = subprocess.run(["chdman"], capture_output=True, text=True, timeout=5)
    ...
    def get_old_entries(self, days: int = 90) -> list:
        ...
        return self.session.query(ROMCache).filter(ROMCache.last_used < cutoff).all()
    ...
    def clear(self, format: str | None = None) -> int:
        """Clear cache entries.
