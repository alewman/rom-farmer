...
from romfarmer.catalog.database import DatGame, RomGroomerDatabase
...
class RomScanResult:
    ...
    md5: str | None = None
    sha1: str | None = None
    matched_game: DatGame | None = None
...
class RomScanner:
    ...
    def scan_directory(
        ...
        logger.info(
            f"Scan complete: {stats.files_matched}/{stats.total_files_scanned} files matched "
    ...
    def _scan_file(self, file_path: Path, dat_name: str | None = None) -> RomScanResult | None:
        """
    ...
    def _calculate_hashes(self, file_path: Path) -> tuple[str, str | None, str | None]:
        """
    ...
    def generate_report(
        ...
        lines.append(
            f"  Match Rate: {stats.files_matched / max(stats.total_files_scanned, 1) * 100:.1f}%"
        )
