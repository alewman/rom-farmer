...
class WikiSearch:
    ...
    def __init__(self, data_dir: Optional[Path] = None):
        if data_dir is None:
            data_dir = Path(__file__).parent.parent.parent.parent / "data" / "wikipedia" / "processed"
    ...
    def get_game_info(self, game_title: str) -> Optional[dict]:
        """
    ...
    def get_section(self, game_title: str, section_name: str) -> Optional[str]:
        """
        ...
        result = self.duck_conn.execute("""
            ...
            ORDER BY chunk_index
        """, [f"%{game_title}%", f"%{section_name}%"]).fetchall()
    ...
    def find_games(
        ...
        ilike_any = ' OR '.join(
            [f"LOWER(game_title) LIKE '%{w}%'" for w in escaped_words]
        )
        ...
        results = self.duck_conn.execute(f"""
            SELECT * FROM (
                SELECT DISTINCT 
                    game_title,
                    article_title,
            ...
            LIMIT ?
        """, [query_lower, threshold, limit * 3]).fetchall()
...
def get_game_info(game_title: str) -> Optional[dict]:
    """Quick game lookup."""
