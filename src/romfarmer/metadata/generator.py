...
from .database import MediaType, MetadataDatabase, ScrapedGame
...
class GamelistGenerator:
    ...
    def _resolve_media_types(self, media_types: list[str] | None) -> list[str]:
        """
    ...
    def _match_m3u_playlist(self, m3u_file: Path, roms_dir: Path) -> ScrapedGame | None:
        """
            ...
            with open(m3u_file, encoding="utf-8") as f:
                for line in f:
                    ...
                    if first_disc_path.exists():
                        ...
                        console.print(
                            f"[yellow]⚠ First disc not found for {m3u_file.name}: {first_disc_name}[/yellow]"
                        )
                        return None
    ...
    def _find_game_via_transformation(self, final_md5: str) -> ScrapedGame | None:
        """
        ...
        transformation = session.query(ROMTransformation).filter_by(final_md5=final_md5).first()
    ...
    def _add_text_element(self, parent: ET.Element, tag: str, text: str | None) -> None:
...
def print_generation_stats(stats: GenerationStats) -> None:
    ...
    match_rate = (stats.roms_matched / stats.roms_found * 100) if stats.roms_found > 0 else 0
