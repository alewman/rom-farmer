...
class TransformationRecorder:
    ...
    def __init__(self, session: Session, dat_manager: DATManager | None = None):
        """
    ...
    def record_transformation(
        ...
        console.print("[cyan]Capturing source file hashes...[/cyan]")
        source_hashes = self.hash_capture.get_source_hashes(source_file, system=system)
        ...
        if source_hashes.from_dat:
            ...
            console.print(
                f"  [yellow]⚠ Source hashes calculated ({source_hashes.calculation_time:.1f}s)[/yellow]"
            )
        ...
        try:
            ...
            if context.final_file is None:
                console.print(
                    "[yellow]⚠ Warning: Final file not set, skipping transformation record[/yellow]"
                )
                return
    ...
    def _save_transformation(self, context: TransformationContext):
        ...
        existing = (
            self.session.query(ROMTransformation)
            .filter_by(
        ...
        if existing:
            console.print(
                f"[yellow]  ⚠ Transformation already exists (id={existing.id}), updating...[/yellow]"
            )
            ...
            transformation = ROMTransformation(
                ...
                source_dat=context.source_hashes.dat_name
                if context.source_hashes.from_dat
                else None,
            ...
            console.print("[green]  ✓ Created new transformation record[/green]")
        ...
        console.print(
            f"[green]  ✓ Transformation saved to database (id={transformation.id})[/green]"
        )

    def find_source_hash(self, final_file: Path) -> ROMTransformation | None:
        """
        ...
        final_hashes = self.hash_capture.get_source_hashes(
            final_file, system=None, force_calculate=False
        )

        # Look up transformation by final hash
        transformation = (
            self.session.query(ROMTransformation).filter_by(final_md5=final_hashes.md5).first()
        )
        ...
        if transformation:
            ...
            console.print("[yellow]  ⚠ No transformation found for this file[/yellow]")
            return None
    ...
    def link_to_game(self, transformation: ROMTransformation, game_name: str, system: str):
        ...
        game = self.session.query(ScrapedGame).filter_by(name=game_name, system=system).first()
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_duration": (
                ...
                if self.stats["transformations_recorded"] > 0
                else 0.0
            ),
