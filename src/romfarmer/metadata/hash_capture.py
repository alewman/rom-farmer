...
class SourceHashInfo:
    ...
    dat_name: str | None = None  # Which DAT?
...
class SmartHashCapture:
    ...
    def __init__(self, dat_manager: DATManager | None, session: Session):
        """
    ...
    def get_source_hashes(
        self, file_path: Path, system: str | None = None, force_calculate: bool = False
    ) -> SourceHashInfo:
        """
    ...
    def _lookup_cache(self, file_path: Path, file_size: int) -> SourceHashInfo | None:
        """
    ...
    def _calculate_hashes(self, file_path: Path, file_size: int) -> SourceHashInfo:
        ...
        if show_progress:
            with Progress(
                ...
                progress.add_task(f"Calculating hashes for {file_path.name}...", total=None)
    ...
    def _store_cache(self, file_path: Path, file_size: int, hashes: SourceHashInfo):
        ...
        cached = self.session.query(HashCache).filter_by(file_path=str(file_path)).first()
    ...
    def get_stats(self) -> dict:
        ...
        total = sum([self.stats["dat_hits"], self.stats["cache_hits"], self.stats["calculated"]])
    ...
    def print_stats(self):
        ...
        if stats["calculated"] > 0:
            console.print(f"  Avg Calculation Time: {stats['avg_calculation_time']:.1f}s")
            console.print(f"  Total Calculation Time: {stats['total_calculation_time']:.1f}s")
