...
class ArcadeFilter:
    ...
    def __init__(
        self,
        config: ArcadeFilterConfig | None = None,
        classifier: ArcadeClassifier | None = None,
    ):
    ...
    def filter_dat(self, dat: DATFile) -> list[FilteredGame]:
        ...
        if self.config.filter_romof:
            ...
            games_to_filter = [
                g for g in games_to_filter if g.romof and g.romof.lower() == romof_pattern
            ]
        ...
        if self.config.exclude_romof:
            ...
            games_to_filter = [
                g for g in games_to_filter if not g.romof or g.romof.lower() not in exclude_patterns
            ]

        # Classify all games
        classified = [(game, self.classifier.classify(game)) for game in games_to_filter]
    ...
    def _filter_group(
        ...
        for game, classification in hacks:
            if self.config.mode == ArcadeFilterMode.ALL:
                ...
                if classification.importance in (
                    CloneImportance.ESSENTIAL,
                    CloneImportance.RECOMMENDED,
                ):
        ...
        for game, classification in bootlegs:
            ...
            if classification.importance in (
                ...
                if self.config.include_hacks or self.config.mode in (
                    ...
                    selected = (
                        classification.is_working if self.config.include_working_only else True
                    )
