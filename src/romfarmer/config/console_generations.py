"""Console generation definitions (platform → generation vocabulary).

Moved from ``romfarmer.ai.generation`` (2026-09-03) so that ``config`` — and
therefore the compiler driver — never imports from ``ai``.  ``ai.generation``
re-exports these names for existing callers.

Used for 1R1G cross-platform deduplication and for the optimizer's
per-generation rating thresholds.
