...
class TargetOverrides(BaseModel):
    ...
    additional_unsupported: List[str] = Field(
        ...
        description="Additional platforms to exclude for this specific target"
    )
...
class TargetConfig(BaseModel):
    ...
    name: str = Field(
        description="Target identifier (e.g., 'rocknix-r36s')"
    )
    ...
    frontend: str = Field(
        description="Frontend name (must match a frontend config file)"
    )
    device: str = Field(
        description="Device name (must match a device config file)"
    )
    
    # Target-specific overrides
    overrides: TargetOverrides = Field(
        default_factory=TargetOverrides,
        description="Target-specific overrides"
    )
...
class ComposedTarget(BaseModel):
    ...
    def supports_platform(self, platform: str) -> bool:
        ...
        if platform.lower() in [p.lower() for p in self.target_config.overrides.additional_unsupported]:
            return False
    ...
    def get_extensions(self, platform: str) -> Optional[List[str]]:
