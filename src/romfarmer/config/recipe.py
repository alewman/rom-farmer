...
class RecipeSpec(BaseModel):
    ...
    # List application
    apply_lists: Optional[bool] = Field(
        None,
    ...
    metadata: Optional[bool] = Field(
        None,
        description="Whether to generate metadata (gamelist.xml). "
                    "None = use target default."
    )
    
    # Arcade-specific overrides
    arcade_filter_overrides: Optional[Dict[str, Any]] = Field(
        None,
        description="Override arcade filter settings from platform config"
    )
    
    # PS3 DLC override
    ps3_dlc_enabled: Optional[bool] = Field(
        None,
        description="Override PS3 DLC processing. None = use platform default."
    )
    ps3_updates_enabled: Optional[bool] = Field(
        None,
        description="Override PS3 updates. None = use platform default."
    )
    ...
    def validate_compression(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            if v.lower() not in valid:
                raise ValueError(
                    f"Invalid compression format '{v}'. "
    ...
    def get_effective_compression(self) -> Optional[CompressionFormat]:
