...
def _make_entry(name: str, rating: float = 0.8, generation: str = "gen6",
                size_bytes: int = 1_000_000_000) -> dict:
    return dataclasses.asdict(PoolEntry(
        name=name, rating=rating, generation=generation,
        size_bytes=size_bytes, platform="psx",
    ))
...
class TestMergeEntries:
    ...
    def test_existing_wins_on_conflict(self):
        ...
        existing = [{"name": "B", "rating": 0.9, "generation": "gen6",
                     "size_bytes": 100, "multi_disc_group": None,
                     "is_disc_anchor": True, "platform": "psx"}]
        incoming = [{"name": "B", "rating": 0.5, "generation": "gen6",
                     "size_bytes": 100, "multi_disc_group": None,
                     "is_disc_anchor": True, "platform": "psx"}]
        merged = _merge_entries(existing, incoming)
        assert len(merged) == 1
...
class TestApplyThresholdsTooBuild:
    ...
    def test_writes_thresholds(self, tmp_path):
        ...
        result = apply_thresholds_to_build(
            build_path, {"gen6": 0.85, "gen7": 0.92}
        )
    ...
    def test_zero_thresholds_stripped(self, tmp_path):
        ...
        result = apply_thresholds_to_build(
            build_path, {"gen3": 0.0, "gen4": 0.0, "gen6": 0.85}
        )
    ...
    def test_dry_run_does_not_write(self, tmp_path):
        ...
        result = apply_thresholds_to_build(
            build_path, {"gen6": 0.85}, dry_run=True
        )
...
class TestApplyThresholdsToBuilds:
    def test_applies_to_multiple(self, tmp_path):
        ...
        for name in ["build-a", "build-b"]:
            (builds_dir / f"{name}.yaml").write_text(
                yaml.dump({"name": name, "recipes": ["r"]})
            )
    ...
    def test_missing_build_skipped(self, tmp_path):
        builds_dir = tmp_path / "builds"
        builds_dir.mkdir()
        (builds_dir / "exists.yaml").write_text(
            yaml.dump({"name": "exists", "recipes": ["r"]})
        )
