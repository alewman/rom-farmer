...
def test_offset_calculation():
    ...
    print(
        f"{'Offset':<8} {'Percentage':<12} {'Expected Range':<20} {'Calculated Range':<20} {'Status'}"
    )
    print("=" * 80)
    ...
    for offset, percentage, expected_start, expected_end in test_cases:
        ...
        print(
            f"{offset:<8.1f} {percentage:<12.1f} {expected_start}-{expected_end:<14} {calc_start}-{calc_end:<14} {status}"
        )
...
def test_phased_coverage():
    ...
    for i in range(len(phases) - 1):
        if phases[i][1] != phases[i + 1][0]:
            print(
                f"❌ GAP detected between Phase {i + 1} and Phase {i + 2}: {phases[i][1]} != {phases[i + 1][0]}"
            )
            has_gaps = True
