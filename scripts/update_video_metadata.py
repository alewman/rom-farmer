...
def main():
    ...
    with db.get_session() as session:
        ...
        needs_update = sum(1 for v in videos if v.video_codec is None or v.video_bitrate is None)
        ...
        with Progress(
            ...
            for video in videos:
                ...
                if (
                    ...
                    and video.video_bitrate is not None
                    and video.video_fps is not None
                    and video.video_duration is not None
                ):
                    skipped += 1
                    continue
    ...
    with db.get_session() as session:
        ...
        results = (
            session.query(MediaFile.video_codec, func.count(MediaFile.id).label("count"))
            .filter(MediaFile.media_type == "video", MediaFile.video_codec.isnot(None))
    ...
    with db.get_session() as session:
        ...
        stats = (
            ...
            .filter(MediaFile.media_type == "video", MediaFile.video_bitrate.isnot(None))
            .first()
        )
