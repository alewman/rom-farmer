...
def main():
    ...
    unlinked = (
        session.query(ROMTransformation)
        .filter(ROMTransformation.game_id is None, ROMTransformation.source_file_name.like("%.cue"))
        .limit(5)
        .all()
    )
    ...
    for i, trans in enumerate(unlinked, 1):
        ...
        game = (
            ...
            .filter(ScrapedGame.system == "saturn", ScrapedGame.md5 == trans.source_md5)
            .first()
        )

        if game:
            print(f"   ✅ Game DOES exist in DB: {game.name}")
            print("      (This means MD5s match but linking failed)")
        else:
            print("   ❌ Game NOT in ARRM database with this MD5")
            print("      (ARRM has different MD5 for this game)")
