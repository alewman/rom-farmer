...
# main() runs the 3D Baseball case
TEST_HASHES = TEST_HASHES_3D_BASEBALL
...
""")
    ...
    results["chd_yours"] = test_hash(
        client, TEST_HASHES["chd_yours"], "Your CHD", "3D Baseball (USA)"
    )
    ...
    results["bin_full"] = test_hash(
        client, TEST_HASHES["bin_full"], "Full BIN", "3D Baseball (USA)"
    )
    ...
    results["bin_track1"] = test_hash(
        client, TEST_HASHES["bin_track1"], "BIN Track 1", "3D Baseball (USA)"
    )
    ...
    print(
        f"CUE Hash:                 {'✅ FOUND' if results.get('cue') else '❌ NOT FOUND'} ← Transform approach ⭐"
    )
    print(f"Full BIN Hash:            {'✅ FOUND' if results.get('bin_full') else '❌ NOT FOUND'}")
    print(
        f"BIN Track 1 Hash:         {'✅ FOUND' if results.get('bin_track1') else '❌ NOT FOUND'}"
    )
