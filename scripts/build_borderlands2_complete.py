...
def main():
    ...
    print("\n💡 HYBRID Build:")
    print("   - Updates: Baked into game (v01.15)")
    print("   - DLC: Extracted for real PS3 + PKGs for RPCS3")
    print("   - PKG files stored in: _PKG/ (at root, ignored by real PS3)")
    ...
    if stage.apply_dlc:
        ...
        if dlc_list:
            ...
            print("   - Extracting major campaign DLC to PS3_GAME/USRDIR/DLC/ (for real PS3)")
            print(f"   - Copying ALL {len(dlc_list)} PKG files to _PKG/ (for RPCS3 - at root)")
            print("   - Downloading RAP files to _PKG/RAPS/ (for RPCS3)")
            ...
            for dlc in dlc_list:
                ...
                is_major_campaign = any(
                    x in dlc_name.upper() for x in ["SCARLETT", "TORGUE", "HAMMERLOCK", "TINA"]
                )
                ...
                if not pkg_source and pkg_url:
                    ...
                    safe_filename = (
                        dlc_name.replace("/", "-").replace(":", "").replace("™", "") + ".pkg"
                    )
                ...
                rap_hash = dlc.get(
                    "RAP", ""
                )  # This is the RAP hex string (16 bytes = 32 hex chars)
    ...
    # Calculate sizes
    ps3_game_size = (
        get_folder_size(output_game / "PS3_GAME") if (output_game / "PS3_GAME").exists() else 0
    )
    ...
    if template_path.exists():
        ...
        print("   ✓ Created README.txt in game folder")
    else:
    ...
    print("    2. Launch - major campaign DLC loads from PS3_GAME/USRDIR/DLC/")
    print("    3. Already at v01.15 (no update needed)")
    print("    4. For cosmetic/character DLC: Install PKGs from _PKG/ folder")
    print("\n  RPCS3:")
    print(f"    1. Boot {output_game.name} in RPCS3")
    print("    2. Install ALL DLC via File → Install PKG")
    print("    3. PKG files in: _PKG/ (at root of game folder)")
    print("    4. Copy RAP files from _PKG/RAPS/ to RPCS3's dev_hdd0/home/00000001/exdata/")
    print("    5. Game already at v01.15")
