...
def main():
    ...
    with db.get_session() as session:
        # Get all image files
        images = session.query(MediaFile).filter(MediaFile.media_type.in_(image_types)).all()

        console.print(f"Found {len(images):,} image files")

        # Count how many need updating
        needs_update = sum(1 for img in images if img.image_mode is None)
    ...
    with db.get_session() as session:
        ...
        results = (
            session.query(MediaFile.image_mode, func.count(MediaFile.id).label("count"))
            .filter(MediaFile.media_type.in_(image_types), MediaFile.image_mode.isnot(None))
            .group_by(MediaFile.image_mode)
    ...
    with db.get_session() as session:
        ...
        total = (
            ...
            .filter(MediaFile.media_type.in_(image_types), MediaFile.has_transparency.isnot(None))
            .scalar()
        )

        with_transparency = (
            session.query(func.count(MediaFile.id))
            .filter(MediaFile.media_type.in_(image_types), MediaFile.has_transparency)
            .scalar()
        )
