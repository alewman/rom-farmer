...
def generate_readme(
    ...
    # Calculate sizes
    ps3_game_size = get_folder_size(game_folder / "PS3_GAME") if (game_folder / "PS3_GAME").exists() else 0
...
def process_game(
    ...
    if stage.apply_dlc:
        ...
        if dlc_list:
            ...
            print(f"   - Extracting major campaign DLC to PS3_GAME/USRDIR/DLC/")
            print(f"   - Copying ALL {len(dlc_list)} PKG files to _PKG/")
            print(f"   - Creating RAP files in _PKG/RAPS/")
            
            # Major campaign keywords (game-agnostic heuristics)
            campaign_keywords = [
                'CAMPAIGN', 'MISSION', 'CHAPTER', 'EPISODE', 'EXPANSION',
                'ASSAULT', 'QUEST', 'ADVENTURE', 'STORY',
                # Borderlands
                'PIRATES', 'DRAGON KEEP', 'TORGUE', 'HAMMERLOCK', 'SCARLETT', 'TINA',
                # Other games
                'LEFT BEHIND', 'UNCHARTED', 'INFAMOUS',
            ]
...
def main():
    ...
    parser.add_argument(
        '--filter',
        type=str,
        help='Only process games starting with this letter (e.g., "A")'
    )
    parser.add_argument(
        '--game',
        type=str,
        help='Only process game with this name (partial match)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    parser.add_argument(
        '--no-updates',
        action='store_true',
        help='Skip update application'
    )
    parser.add_argument(
        '--no-dlc',
        action='store_true',
        help='Skip DLC processing'
    )
    ...
    game_folders = sorted([
        f for f in args.source.iterdir()
        if f.is_dir() and f.suffix == '.ps3'
    ])
