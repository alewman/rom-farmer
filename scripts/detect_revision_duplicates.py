...
def print_report(duplicates: Dict[str, List[Path]], output_dir: Path):
    ...
    print(f"\n{'='*80}")
    print(f"REVISION DUPLICATE ANALYSIS")
    ...
    for base_name, files in duplicates.items():
        ...
        for f in files:
            ...
            if re.search(r'\(Rev [0-9A-Z]+\)', filename):
                if '(Rev' not in filename.replace(re.search(r'\(Rev [0-9A-Z]+\)', filename).group(), ''):
    ...
    if mixed_m3u:
        ...
        for m3u_name, discs in sorted(mixed_m3u.items()):
            ...
            for disc in discs:
                ...
                has_revision = any(re.search(pattern, disc) for pattern in [
                    r'\(Rev [0-9A-Z]+\)',
                    r'\((FA|SA)[A-Z][TS]\)',
                    r'\(Alt [0-9]+\)',
                    r'\([A-Z]{4}(?:, [A-Z]{4})+\)',
    ...
    if multi_disc:
        print(f"\n\nMulti-Disc Games ({len(set(g.split(' (Disc')[0] for g in multi_disc))} series):")
        print("-" * 80)
